<?php
   ...
   if (isset($_POST["form-Category-Type"])) $_SESSION["vehiclecategorytypeid"] = $_POST["form-Category-Type"];
   ...
   if (isset($_POST["form-Promo-Code"]))  $_SESSION["PromoCode"] = $_POST["form-Promo-Code"];
   if (isset($_POST["refid"]))  $_SESSION["refid"] = $_POST["refid"];

   if (isset($_GET["CTypeID"])) $_SESSION["vehiclecategorytypeid"] = $_GET["CTypeID"];
   if (isset($_GET["PID"]))   $_SESSION["PickupLocationID"] = $_GET["PID"];
   if (isset($_GET["DID"]))   $_SESSION["DropOffLocationID"] = $_GET["DID"];
   if (isset($_GET["PDate"])) $_SESSION["PickupDate"] = $_GET["PDate"];
   if (isset($_GET["PTime"])) $_SESSION["PickupTime"] = $_GET["PTime"];
   if (isset($_GET["DDat"]))  $_SESSION["ReturnDate"] = $_GET["DDat"];
   if (isset($_GET["DTime"])) $_SESSION["ReturnTime"] = $_GET["DTime"];
   if (isset($_GET["AgeID"])) $_SESSION["Age"] = $_GET["AgeID"];
   if (isset($_GET["Promo"])) $_SESSION["PromoCode"] = $_GET["Promo"];
   if (isset($_GET["refid"])) $_SESSION["refid"] = $_GET["refid"];
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <link rel="stylesheet" href="//maxcdn.bootstrapcdn.com/font-awesome/4.3.0/css/font-awesome.min.css">
   ...
   <script src="https://{Server}/api/3.2/main/{APIKey}" type="text/javascript"></script>
      ...
      if (document.getElementById("PickupDate").value) {
       ...
       for (var i = 0; i < rcmAvailableCars.length; ++i) {
        ...
        CatTypeID = rcmAvailableCars[i]["vehiclecategorytypeid"];
         ...
         if (oldCatTypeID != CatTypeID) {
           oldCatTypeID = CatTypeID;
           CatType = oAPI.GetCategoryType(CatTypeID);
           out = out + "<div class='row topdiv TopPad cartypetitle'><span>" + CatType + "</span></div>";
         }
         ...
         out = out + "<div class='row'><div class='col-sm-12 text-center'><img src='" + rcmAvailableCars[i]["imageurl"] + "' class='img-responsive' alt='' /></div></div>";
         ...
         if (LocAvailable == true && CarAvailable == true) {
          ...
          if (rcmAvailableCars[i]["available"] == 2)  {
            out = out + "<p class='RemovePadding_p'><span class='red'><b>" + rcmAvailableCars[i]["availablemessage"] + "</b></span></p>";
          }
        ...
        if (LocAvailable == true && CarAvailable == true) {
          out = out + "<div class='row'><div class='col-sm-12 col-xs-7 text-right' id='totalCostFinal'><span class='results-price'> TOTAL &nbsp;  " + CurrencyName + " " + CurrencySymbol + "" + parseFloat(total).toFixed(2) + "</span></div>";
        out = out + "<div class='col-sm-12 col-xs-5 text-right' Id='dvbooknow'><button class='btn' onclick='booknow(" + rcmAvailableCars[i]["carrateid"] + "," + rcmAvailableCars[i]["vehiclecategoryid"] + ")'><span class='glyphicon glyphicon-ok'></span>&nbsp;Select</button></div></div>";
         }
         out = out + "</div>";
              ...
              <form id="frmStep2" role="form" name="frmStep2" action="step3.php" method="post">
                <div class="row">
                  ...
                  <div class="col-sm-3 margin0">
                    <select id="cmbPickup" name="form-Pickup-Location" class="col-sm-12 form-control" onchange="LocUpdForm()"></select>
                  </div>
                  <div class="col-sm-2 margin0">
                    <label class="col-sm-12 margin0" for="form-Pickup-Date">Date & Time: </label>
                  </div>
                  <div class="col-sm-3 margin0">
                    <input type="text" name="form-Pickup-Date" value='<?php echo $_SESSION["PickupDate"]; ?>' placeholder="Pickup-Date..." class="form-control col-sm-12" id="txtPickup" autocomplete="off" onchange="LocUpdForm()">
                  </div>
                  <div class="col-sm-2 margin0">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-3 margin0">
                    <select id="cmbDropOff" name="form-Dropoff-Location" class="col-sm-12 form-control" onchange="LocUpdForm()"></select>
                  </div>
                  <div class="col-sm-2 margin0">
                    <label class="col-sm-12 margin0" for="form-Dropoff-Date">Date & Time: </label>
                  </div>
                  <div class="col-sm-3 margin0">
                    <input type="text" name="form-Dropoff-Date" value='<?php echo $_SESSION["ReturnDate"]; ?>'  placeholder="Dropoff-Date..." class="form-control col-sm-12" id="txtReturn" autocomplete="off" onchange="LocUpdForm()">
                  </div>
                  <div class="col-sm-2 margin0">
